...
```{r}
iris_species_model <- ore.odmNB(SPECIES ~ SEPAL_WIDTH + PETAL_LENGTH + PETAL_WIDTH + SEPAL_LENGTH, IRIS_R)
...
iris_species_predict <- function(dat, ds, obj) {
  ...
  mod <- get(obj) # unneeded? Just reference 'obj'
  dat <- ore.frame(dat) # somewhat uncertain as to the necessity of this

  prd <- predict(mod, dat,supplemental.cols="SPECIES")
  ore.pull(prd) # might not be needed?
}
```
...
In SQL, we can call this script using the `rqTableEval` function. The input to this function is fairly involved, not to mention unintuitive. Let's start by calling the function:
