...
```{sql, connection=con}
ALTER TABLE nycpay
ADD pers_id AS ora_hash(firstname||initial||lastname);
```
...
We can see that out of 2.7 million rows, over 1 million belongs to people who have had more than one job in one year. And that there are 175 675 people who have had more than one job. If our goal was to conduct a thorough review of the city's pay data, we would have to choose to aggregate the data in some way in order to construct a dataset that contained every person and a meaningful unique identifier.
...
CREATE TABLE simplepay AS (
  SELECT year, 
  pers_id,
  ltrim(rtrim(upper(agency))) AS agency, 
  TO_DATE(job_start, 'MM/DD/YYYY') AS job_start,
  LTRIM(RTRIM(job_location)) AS job_location,
  LTRIM(RTRIM(job_title)) AS job_title,
  LTRIM(RTRIM(leave_status)) AS leave_status,
  base_salary,
  LTRIM(RTRIM(pay_basis)) AS pay_basis,
  regular_hours,
  regular_gross_pay,
  ot_hours,
  total_ot_paid,
  ...
  FROM nycpay
  WHERE 
  ltrim(rtrim(upper(job_title))) != 'ELECTION WORKER' AND
  pers_id NOT IN(
    SELECT DISTINCT pers_id FROM nycpay
    WHERE ltrim(rtrim(upper(job_title))) != 'ELECTION WORKER'
    GROUP BY year, pers_id
    HAVING COUNT(1)>1
  )
)
